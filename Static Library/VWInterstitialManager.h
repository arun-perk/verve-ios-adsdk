...
__deprecated_msg("VWInterstitialManager has been deprecated and will be removed in future versions. Please use VWInterstitialAdManager instead.")
 ...
 * set this property according to the current state of an interstitial.
 */
@property (nonatomic, getter=isInterstitialPresenting) BOOL interstitialPresenting;
...
/*
 * When set to YES webview inside of an interstitials will not be scrollable.
 ...
 *
 * An intersitital transition must always have positive, non-zero content category.
 *
 * Include display block ID only if you've got it.
 *
 * Include partner module ID if you have a set of values assigned for your content.
 *
 */
- (void)transitionWithContentCategoryID:(VWContentCategory)contentCategory;
 ...
 */
- (BOOL)interstitialManagerShouldShowDismissControl:(VWInterstitialManager *)manager;

/*
 * If you are doing a custom presentation of an interstitial and you're not hiding interstitial's dismiss control,
 * you must provide custom dismissal mechanism by implementing this method. This method will be called when the user
 * taps dismiss button on an interstitial.
 *
 * Note: It is imperative that you set interstitialPresenting property to NO so that library knows you
 * successfully dismissed an interstitial.
 */
- (void)dismissInterstitialViewControllerVerveInterstitialManager:(VWInterstitialManager*)manager;
